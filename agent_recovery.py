...
2. Walk the graph state by state. At each state, look at every
   available outgoing edge (tool), ordered by:
   (a) whether it matches the candidate plan,
   then (b) TrustMemory's combined score.

3. Execute the chosen tool through the FailureInjector
   (so the same tools.py functions run, but a configured failure
...
4. Classify the raw result with ResultClassifier:
      ...
      EXPLICIT_FAILURE -> reject the result, record the failure,
                           blacklist that (state, tool) edge,
                           try the next candidate edge.
      IMPLICIT_FAILURE -> same handling as EXPLICIT_FAILURE.
      NON_PROGRESS     -> same handling and blacklist the edge.
...
def _real_tool_call(from_state: str, tool_name: str, value: Any) -> Any:
    """
    Execute the real tools.py function for a graph edge.
    """
...
class RecoveryAgent:
    ...
    def run(self, task: Task) -> Dict[str, Any]:
        ...
        # --------------------------------------------------------------
        # Get candidate plan from SearchEngine
        # --------------------------------------------------------------
        ...
        if candidate_path:
            for from_state, tool_name, _to_state in candidate_path:
                preferred_tool_by_state.setdefault(
                    from_state,
                    tool_name
                )
        ...
        record["success"] = task.is_goal_reached(
            record["final_state"]
        )
    ...
    def _walk(
        ...
        for edge in edges:
            ...
            if classification.category is ResultCategory.PROGRESS:
                ...
                self.trust_memory.record_progress(
                    edge.tool_name
                )

                record["states_reached"].append(
                    edge.to_state
                )
                ...
                # Deeper branch failed
                visited_on_branch.discard(
                    edge.to_state
                )
            ...
            self.trust_memory.record_failure(
                edge.tool_name,
            ...
            # Never retry the same failed edge
            self._blacklisted_edges.add(
                edge_key
            )

            # Try next available edge
            continue
    ...
    def _ordered_edges(
        ...
        """
        Rank the edges available from `state`.

        The candidate-plan tool is preferred first.
        Remaining tools are ordered using TrustMemory.
        """

        edges = self.graph.get_next_options(state)

        preferred_tool = preferred_tool_by_state.get(
            state
        )
        ...
        def sort_key(edge: Edge):
            ...
            is_preferred = (
                edge.tool_name != preferred_tool
            )

            trust_score = (
                self.trust_memory.get_combined_score(
                    edge.tool_name
                )
            )
    ...
    def _call_tool(
        ...
        """
        Run a tool through FailureInjector and capture exceptions.
        """

        real_func = lambda v: _real_tool_call(
            from_state,
            tool_name,
            v
        )
        ...
        try:
            result = self.failure_injector.execute(
                tool_name,
                real_func,
                value
            )
...
if __name__ == "__main__":
    ...
    print(
        "== Run 1: no injected failures "
        "(should succeed via the normal path) =="
    )
    ...
    print(
        "\n== Run 2: get_return_id is broken -> "
        "agent must recover via the alternative path =="
    )
    ...
    agent_with_failure = RecoveryAgent(
